...
Uma PWA normal precisa ser disponibilizada por **HTTPS ao menos no primeiro acesso**. `localhost` é aceito apenas para desenvolvimento. Publique o conteúdo de `dist/` em uma hospedagem estática HTTPS, mantendo os arquivos no mesmo domínio e, nesta configuração, na raiz do site.
